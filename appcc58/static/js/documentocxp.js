function colocaPorcentaje(valor, natural, juridico) {
    // Aquí puedes utilizar los valores de "natural" y "juridico"

    document.getElementById('retencion').value = natural.replace(',','.')
    // ...
  }
...
function refreshTableRecibos(idmedico) {
  ...
  function agregarRecibo() {
    ...
    const rows = table.querySelectorAll('tr input[type="checkbox"]:checked').forEach((checkbox) => {
      ...
      rowData.push(`<td ><input type="text" name="id_factura_dolar" value="${montoDolar.textContent.trim()}" style="text-align: right;" ></td>`); 
      ...
      const deleteButtons = document.querySelectorAll(".delete-button");
        deleteButtons.forEach(button => {
          button.addEventListener("click", function() {
            ...
            const montoInputs = tableBody.querySelectorAll('tr td:nth-child(5) input'); // asumiendo que la columna "monto" es la sexta columna
            const montoInputsDolar = tableBody.querySelectorAll('tr td:nth-child(6) input')
  ...
  const totalElement = document.getElementById('total-monto');
  const totalElementDl = document.getElementById('total-monto-dl');
  totalElement.textContent = `${suma.toFixed(2)}`; // mostrar el total
  totalElementDl.textContent = `${sumaDolar.toFixed(2)}`; // mostrar el total
  document.getElementById('baseimponible').value = suma.toFixed(2)
  document.getElementById('baseimponible_usd').value = sumaDolar.toFixed(2)
