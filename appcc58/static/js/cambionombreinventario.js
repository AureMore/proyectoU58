function cambioNombreInventario() {
    ...
    document.getElementById('totalcantidad_compra').value = 0;
    ...
    if (producto_id != '') {
        ...
        var moneda_referencia = document.getElementById('moneda_factura').value
      ...
      var nombre_principal = document.getElementById('exampleDataList').value.trim()
      if (producto_id == '' && nombre_principal != '') {
        activarCamposNoModificables()
        Swal.fire({
          title: '¡Atención!',
          text: 'Este producto no existe en el inventario, se creara automaticamente',
          width: '400px',
          icon: 'warning',
...
function totalCantidadSalida(select) {
  ...
  const factor = parseFloat(selectedOption.dataset.cantidad).toFixed(2);
  const unidades_entrada = parseFloat(document.getElementById('totalcantidad_compra').value).toFixed(4)

  document.getElementById('factor').value = parseFloat(unidades_entrada * factor ) .toFixed(2) */

  /* const selectedOption = select.options[select.selectedIndex];
  console.log('select clasificacion',selectedOption.dataset.clasificacion ) */
}
